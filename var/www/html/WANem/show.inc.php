	...
	for ($i=1; $i<=count($interfaces);++$i) {
		...
		//echo "sym[$i-1] =", $sym[$i-1];
                  ...
                  <tr>
                    ...
                    <tr>
                        ...
                        <td nowrap width="40%">
                                ...
                                <option value=<?=getVal($advflag, $bandwidth[$i-1])?>><?php echo linkName($advflag, $bandwidth[$i-1]); ?></option>
				...
				<?php if ($bandwidth[$i-1] != "1304000") {?> <option value="1304000"><?php echo "OC-24 - 1.244 Gbps"; ?></option><?php } else {?>  <option value="Other"><?php echo "Other"; ?></option>  <?php } ?>
                	...
                	<td width="10%"><input type="text" name="txtBandwidth<?=$i?>" size="7" value=<?php echo $bandwidth[$i-1] ?>></td>
                        <td nowrap width="10%">Delay time(ms) </td>
                        <td nowrap width="10%"><input type="text" name="txtDelay<?=$i?>" size="7" value=<?php echo $del[$i-1] ?>></td>
                   </tr>
                   </table>
                   </div>
                   </center>
...
?>
	...
	<table border="1" align="center" width="100%">
		...
		<td width="50%" colspan="5" align="center"><b>Packet Limit <input type="text" name="txtLimit<?=$i?>" size="9" value=<?php echo $limit[$i-1] ?>></b> (Default=1000)
		</td>
		...
		<td width="10%" ><input type="text" name="txtDelay<?=$i?>" size="7" value=<?php echo $del[$i-1] ?>></td>
		<td width="10%" >Loss(%)</td>
		<td width="10%" ><input type="text" name="txtLoss<?=$i?>" size="7" value=<?php echo $loss[$i-1] ?>></td>
		<td width="10%" >Duplication(%)</td>
		<td width="10%" ><input type="text" name="txtDup<?=$i?>" size="7" value=<?php echo $dup[$i-1] ?>></td>
		<td width="10%" >Reordering(%)</td>
		<td width="10%" ><input type="text" name="txtReorder<?=$i?>" size="7" value=<?php echo $reorder[$i-1] ?>></td>
		<td width="10%" >Corruption(%)</td>
		<td width="10%" ><input type="text" name="txtCorrupt<?=$i?>" size="7" value=<?php echo $corrupt[$i-1] ?>></td>
		</tr>
		<tr>
		<td width="10%" >Jitter(ms)</td>
		<td width="10%" ><input type="text" name="txtDelayJitter<?=$i?>" size="7" value=<?php echo $delJitter[$i-1] ?>></td>
		<td width="10%" >Correlation(%)</td>
		<td width="10%" ><input type="text" name="txtLossCorrelation<?=$i?>" size="7" value=<?php echo $lossCorrelation[$i-1] ?>></td>
		<td width="10%" >Correlation(%)</td>
		<td width="10%" ><input type="text" name="txtDupCorrelation<?=$i?>" size="7" value=<?php echo $dupCorrelation[$i-1] ?>></td>
		<td width="10%" >Correlation(%)</td>
		<td width="10%" ><input type="text" name="txtReorderCorrelation<?=$i?>" size="7" value=<?php echo $reorderCorrelation[$i-1] ?>></td>
		</tr>
		<tr>
		<td width="10%" colspan="1" >Correlation(%)</td>
		<td width="10%" colspan="1" ><input type="text" name="txtDelayCorrelation<?=$i?>" size="7" value=<?php echo $delCorrelation[$i-1] ?>></td>
		<td width="10%" colspan="4" ></td>
		<td width="10%" colspan="1" >Gap(packets)</td>
		<td width="10%" colspan="1" ><input type="text" name="txtGap<?=$i?>" size="7" value=<?php echo $gap[$i-1] ?>></td>
		</tr>
		<tr>
		<td width="10%" >Distribution</td>
		<td width="10%" ><select size="1" name="selDelayDistribution<?=$i?>">
		  <option <?php if($delDistribution=="0") echo "selected"?>>-N/A-</option>
	...
	<table border="1" width="auto" valign=top>
		  <tr>
			...
			<td width="4%"  align=left><input type="text" name="txtidtmr<?=$i?>" size="5" value=<?php echo $disc[$i-1]->idl_timer ?>></td>
			<td width="15%" align=right colspan=3>Disconnect Timer</td>
			<td width="4%" align=left><input type="text" name="txtidsctmr<?=$i?>" size="5" value=<?php echo $disc[$i-1]->idl_disc_timer ?>></td>
		  </tr>
		  <tr>
			...
			<td width="4%" align=left><input type="text" name="txtrndmttflo<?=$i?>" size="5" value=<?php echo $disc[$i-1]->rnd_mttf_lo ?>></td>
			<td nowrap width="10%" align=right>MTTF High</td>
			<td width="4%" align=left><input type="text" name="txtrndmttfhi<?=$i?>" size="5" value=<?php echo $disc[$i-1]->rnd_mttf_hi ?>></td>
			<td nowrap width="10%" align=right>MTTR Low</td>
			<td width="4%" align=left><input type="text" name="txtrndmttrlo<?=$i?>" size="5" value=<?php echo $disc[$i-1]->rnd_mttr_lo ?>></td>
			<td nowrap width="10%" align=right>MTTR High</td>
			<td width="4%" align=left><input type="text" name="txtrndmttrhi<?=$i?>" size="5" value=<?php echo $disc[$i-1]->rnd_mttr_hi ?>></td>
		  </tr>
		  <tr>
			...
			<td nowrap width="10%" align=right>MTTF Low</td>
			<td width="4%" align=left><input type="text" name="txtrcdmttflo<?=$i?>" size="5" value=<?php echo $disc[$i-1]->rcd_mttf_lo ?>></td>
			<td nowrap width="10%" align=right>MTTF High</td>
			<td width="4%" align=left><input type="text" name="txtrcdmttfhi<?=$i?>" size="5" value=<?php echo $disc[$i-1]->rcd_mttf_hi ?>></td>
			<td nowrap width="10%" align=right>MTTR Low</td>
			<td width="4%" align=left><input type="text" name="txtrcdmttrlo<?=$i?>" size="5" value=<?php echo $disc[$i-1]->rcd_mttr_lo ?>></td>
			<td nowrap width="10%" align=right>MTTR High</td>
			<td width="4%" align=left><input type="text" name="txtrcdmttrhi<?=$i?>" size="5" value=<?php echo $disc[$i-1]->rcd_mttr_hi ?>></td>
		  </tr>
		</table>
		<table border="1" align="center" width="auto">
		  <tr>
			<td width="9%" >IP source address</td>
			<td width="11%" ><input type="text" name="txtSrc<?=$i?>" size="13" value=<?php echo $src[$i-1] ?>></td>
			<td width="9%" >IP source subnet</td>
			<td width="11%" ><input type="text" name="txtSrcSub<?=$i?>" size="13" value=<?php echo $srcSub[$i-1] ?>></td>
			<td width="9%" >IP dest address</td>
			<td width="11%" ><input type="text" name="txtDest<?=$i?>" size="13" value=<?php echo $dest[$i-1] ?>></td>
			<td width="9%" >IP dest subnet</td>
			<td width="11%" ><input type="text" name="txtDestSub<?=$i?>" size="13" value=<?php echo $destSub[$i-1] ?>></td>
			<td width="9%" >Application port if any</td>
			<td width="11%" ><input type="text" name="txtPort<?=$i?>" size="13" value=<?php echo $port[$i-1] ?>></td>
		  </tr>
		</table>
	</div>
	</center>
...
?>
		<div style="color: #000000; background-color: #aaccff; border: thin solid #000000; width: 970px;  align: center">
