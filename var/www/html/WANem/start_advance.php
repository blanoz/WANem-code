...
<!--
	...
	<div style="color: #000000; background-color: white; border: #000000; height: 150px; align: center">
...
<?php
				echo "<h1>".count($interfaces)." Interfaces available</h1>";
...
<?php
				...
				$selectHTML = "<p><select size='1' name='selInt'><option selected>";
				$selectHTML .= $interfaces[0] ;
				$selectHTML .= "</option>";
				//Check if there are more interfaces to add
				if (count($interfaces)>0) {
					for ($i=1; $i<count($interfaces);++$i) {
						$selectHTML .= "<option>".$interfaces[$i]."</option>";
					}
				}
				$selectHTML .= "</select></p>" ;
				echo $selectHTML;
